    ...
    <div class="mb-8 flex items-center justify-between gap-2">
      <div class="flex items-center gap-4">
        <div class="h-12 w-12 rounded-full">
          ...
          />
          <div v-else class="h-12 w-12 flex items-center justify-center rounded-full bg-gray-200">
      ...
      <div class="flex gap-1.5 items-center rounded-lg bg-gray-200 py-1 pl-2 pr-3 text-black">
        <svg class="h-4 w-4 text-yellow-500" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path d="M12 .587l3.668 7.431L24 9.168l-6 5.847 1.417 8.255L12 18.897l-7.417 4.373L6 15.015 0 9.168l8.332-1.15z" />
        </svg>
        <span class="text-lg font-semibold">{{ Number(item.author_details.rating).toFixed(1) || 0 }}</span>
      </div>
    </div>
    <div 
      class="line-clamp-8 text-sm leading-relaxed italic text-gray-800" 
      v-html="item.content"
    />
  </div>
</template>
